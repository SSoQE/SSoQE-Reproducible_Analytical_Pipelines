...
The lecture repository contains the slides and a self-contained [`setup_exercise.R`](setup_exercise.R). Participants create a new RStudio Project outside the lecture repository and run the default pipeline generated by `targets::use_targets()` in Exercise 1. At the start of Exercise 2, they source the small bootstrap script from raw GitHub. It writes and verifies the prepared penguin files in one web request without replacing the generated target script. Participants never create `_targets.R` or `_targets/` in the lecture repository.
...
3. Bring a personal working location for the new `targets-penguins` RStudio Project. You will create it during Exercise 1.
4. Check that the raw [`setup_exercise.R`](https://raw.githubusercontent.com/SSoQE/SSoQE-Reproducible_Analytical_Pipelines/main/setup_exercise.R) page opens in your browser. You will source it during Exercise 2, after trying the generated pipeline.
...
Before teaching, publish these exercise files on the public `main` branch and verify the raw bootstrap and Checkpoint 1 links in an unauthenticated browser. A private repository or unpublished branch makes those learner-facing links return 404.

## Exercise route

Follow the exercises in the [presentation](https://ssoqe.github.io/SSoQE-Reproducible_Analytical_Pipelines/), but run every learner command in the separate `targets-penguins` project:

1. **First pipeline:** create a separate project, run `targets::use_targets()`, build the generated example twice, and inspect its graph.
2. **Build:** source the one-request bootstrap into the same project, then replace the generated example with a seven-target penguin pipeline, run it twice, and retrieve its figure.
...
If you fall behind during Exercise 1, use the [raw default-script checkpoint](https://raw.githubusercontent.com/SSoQE/SSoQE-Reproducible_Analytical_Pipelines/main/Exercise_project_starter/Checkpoints/01_default_targets.R). After the Exercise 2 bootstrap, these checkpoints are local in your `targets-penguins` project:
